...
class Surface:
    def __init__(self, r, d, is_square=False, device=torch.device("cuda:0")):
        ...
        self.NEWTONS_TOLERANCE_TIGHT = (
            10e-6  # in [mm], i.e. 10 [nm] here (up to <10 [nm])
        )
        self.NEWTONS_TOLERANCE_LOOSE = (
            50e-6  # in [mm], i.e. 100 [nm] here (up to <10 [nm])
        )
    ...
    def _refract(self, ray, eta, approx=False):
        ...
        sr = torch.sqrt(
            1 - eta**2 * (1 - cosi.unsqueeze(-1) ** 2) * valid.unsqueeze(-1)
        )  # square root term
...
class Aspheric(Surface):
    ...
    def ray_reaction(self, ray, eta, forward=True):
        ...
        if self.c.item() == 0:
            ...
            if self.is_square:
                ...
                width = self.r
                valid = (
                    (torch.abs(new_o[..., 0]) <= height)
                    & (torch.abs(new_o[..., 1]) <= width)
                    & (ray.ra > 0)
                )
            else:
                valid = (
        ...
        # Aspheric surface
            ...
            new_o[~valid] = ray.o[
                ~valid
            ]  # invalid rays will stay in the original positions
            ray.o = new_o
    ...
    def _newtons_method(self, ray, maxt):
        ...
        with torch.no_grad():
            ...
            while (torch.abs(ft) > self.NEWTONS_TOLERANCE_LOOSE).any() and (
                ...
                new_o = ray.o + ray.d * t.unsqueeze(-1)
                new_x, new_y = new_o[..., 0], new_o[..., 1]
                valid = self._valid_loose(new_x, new_y) & (
                    ray.ra > 0
                )  # use _valid_loose here because o may go back after exceeding boundary
                ...
                if torch.sum(torch.isnan(ft)) > 0:
                    ...
                    exit(0)
                dr2dt = 2 * (
                    (ray.d[..., 0] ** 2 + ray.d[..., 1] ** 2) * t
                    + (ray.d[..., 0] * ray.o[..., 0] + ray.d[..., 1] * ray.o[..., 1])
                )
        ...
        t = t - torch.clamp(
            ft / (dfdt + EPSILON), -self.NEWTONS_STEP_BOUND, self.NEWTONS_STEP_BOUND
        )
    ...
    def _valid(self, x, y):
        ...
        if self.k > -1:
            valid = ((x**2 + y**2) < self.r**2) & (
                (x**2 + y**2) < (1 - EPSILON) / self.c**2 / (1 + self.k)
            )
        else:
            valid = (x**2 + y**2) < self.r**2
    ...
    def surface_and_derivatives_dot_D(self, t, dx, dy, dz, ox, oy, z):
        ...
        residual_derivative = (
            self._dgd(r2) * 2 * ((dx**2 + dy**2) * t**2 + (ox + oy) * t) - dz
        )
