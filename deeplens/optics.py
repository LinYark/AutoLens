...
class Lensgroup:
    ...
    def prepare_sensor(self, sensor_res=(512, 512)):
        ...
        self.sensor_res = (H, W)
        self.sensor_size = [
            2 * self.r_last * H / np.sqrt(H**2 + W**2),
            2 * self.r_last * W / np.sqrt(H**2 + W**2),
        ]
    ...
    def find_aperture(self):
        ...
        for i in range(len(self.surfaces) - 1):
            if (
                self.materials[i].A < 1.0003 and self.materials[i + 1].A < 1.0003
            ):  # AIR or OCCLUDER
                self.aper_idx = i
                return
        ...
        if self.aper_idx is None:
            print(
                "There is no aperture stop in the lens, use the first surface as the aperture."
            )
    ...
    def sample_point_source_2D(
        self, depth=-1000, view=0, M=9, entrance_pupil=False, wavelength=DEFAULT_WAVE
    ):
    ...
    def sample_point_source(
        ...
        if pupil:
            ...
            # cone shape rays
            theta = torch.deg2rad(torch.rand(spp, M, M) * fov).to(
                self.device
            )  # [0, fov]
    ...
    def trace2obj(self, ray, depth=DEPTH, ignore_aper=False):
        ...
        (
            ...
            _,
            _,
        ) = self.trace(ray, ignore_aper=ignore_aper)
    ...
    def trace_to_sensor(
        self, ray, ignore_invalid=False, record=False, ignore_aper=False
    ):
        ...
        self.surfaces.append(
            Aspheric(self.r_last, self.d_sensor, 0.0, device=self.device)
        )
    ...
    def _forward_tracing(self, ray, lens_range, record):
        ...
        for i in lens_range:
            eta = self.materials[i].ior(wavelength) / self.materials[i + 1].ior(
                wavelength
            )
    ...
    def psf_map(self, M=7, kernel_size=11, depth=DEPTH, spp=2048):
        ...
        ray = self.sample_point_source(
            M=M, R=self.sensor_size[0] / 2 / mag, depth=depth, spp=spp, pupil=True
        )
        ...
        xy_norm = (
            o2[..., :2] - xy_center_ref
        ) * ra  # invalid rays are counted as (0, 0)
        x_idx, y_idx = (
            xy_norm[..., 0] / self.pixel_size,
            xy_norm[..., 1] / self.pixel_size,
        )
        ...
        for i in range(kernel_size):
            for j in range(kernel_size):
                kernel[..., i, j] = torch.sum(
                    (x_idx == (-kernel_size // 2 + 1 + j))
                    & (y_idx == (kernel_size // 2 - i)),
                    dim=0,
                )

        # remove invalid rays contributions to cener kernel
        kernel[:, :, kernel_size // 2, kernel_size // 2] -= torch.sum(
            ra != 1, dim=0
        ).squeeze(-1)
    ...
    def assign_points_to_pixels(self, points, ks, x_range, y_range, ra):
        ...
        grid.index_put_(
            tuple(pixel_indices_tl.t()), (1 - w_b) * (1 - w_r) * ra, accumulate=True
        )
        grid.index_put_(
            tuple(pixel_indices_tr.t()), (1 - w_b) * w_r * ra, accumulate=True
        )
        grid.index_put_(
            tuple(pixel_indices_bl.t()), w_b * (1 - w_r) * ra, accumulate=True
        )
    ...
    def psf_diff(self, point, wvln=DEFAULT_WAVE, kernel_size=21, spp=4096, center=True):
        ...
        psf_range = [
            (-kernel_size / 2 + 0.5) * self.pixel_size,
            (kernel_size / 2 - 0.5) * self.pixel_size,
        ]
        ...
        ra = (
            ra
            * (points_shift[..., 0].abs() < psf_range[1]).unsqueeze(1)
            * (points_shift[..., 1].abs() < psf_range[1]).unsqueeze(1)
        )
        points_shift *= ra
        ...
        ra = ra[:, 0, 0]
        psf = self.assign_points_to_pixels(
            points=points_shift,
            ks=kernel_size,
            x_range=psf_range,
            y_range=psf_range,
    ...
    def psf_diff_new(
        self, point, wvln=DEFAULT_WAVE, kernel_size=21, spp=512, center=True
    ):
    ...
    def psf_diff_color(self, o, kernel_size=31, spp=4096, center=True):
        ...
        for wvln in WAVE_RGB:
            psf.append(
                self.psf_diff(
                    o, wvln=wvln, kernel_size=kernel_size, spp=spp, center=center
                )
            )
    ...
    def calc_foc_dist(self, wvln=DEFAULT_WAVE, spp=4096):
        ...
        o2 = self.surfaces[0].surface_sample(
            spp
    ...
    def refocus_inf(self):
        ...
        for i in range(3):
            ray = self.sample_parallel_2D(
                R=self.surfaces[0].r, M=M, wavelength=WAVE_RGB[i]
            )
            ray, _, _ = self.trace(ray)
            t = (ray.d[..., 0] * ray.o[..., 0] + ray.d[..., 1] * ray.o[..., 1]) / (
                ray.d[..., 0] ** 2 + ray.d[..., 1] ** 2
    ...
    def refocus(self, depth=DEPTH):
        ...
        for i in range(3):
            ...
            ray = Ray(
                o, d, normalized=False, wavelength=WAVE_RGB[i], device=self.device
            )
            ray, _, _ = self.trace(ray)
    ...
    def calc_fov(self):
        ...
        o1 = torch.zeros([M, 3])
        o1[:, 0] = (
            self.r_last if torch.is_tensor(self.r_last) else torch.Tensor([self.r_last])
        )
        o1[:, 2] = (
            self.d_sensor
            if torch.is_tensor(self.d_sensor)
            else torch.Tensor([self.d_sensor])
        )
    ...
    def calc_magnification3(self, depth):
        ...
        spp = 512
        ray = self.sample_point_source(
            M=M, spp=spp, depth=depth, R=-depth * np.tan(self.hfov) * 0.5, pupil=True
        )
    ...
    def pruning_v2(self, outer=None, surface_range=None):
        ...
        view = (
            ...
            else np.arctan(self.r_last / self.d_sensor)
        )
        ray = self.sample_parallel_2D(
            R=aper, view=np.rad2deg(view), M=11, entrance_pupil=False
        )
    ...
    def analysis(
        ...
        print(
            f"Avg RMS spot size (radius): {round(rms_avg.item()*1000,3)}um, On-axis RMS radius: {round(rms_radius_on_axis.item()*1000,3)}um, Off-axis RMS radius: {round(rms_radius_off_axis.item()*1000,3)}um"
        )

        return rms_avg

    @torch.no_grad()
    def plot_setup2D_with_trace(
        self,
        filename,
        views=[0],
        M=9,
        ...
        if multi_plot:
            ...
            for i, wavelength in enumerate(WAVE_RGB):
                ...
                ax, fig = self.plot_setup2D(
                    ax=ax, fig=fig, show=False, zmx_format=zmx_format
                )

                for view in views:
                    ray = self.sample_parallel_2D(
                        R, wavelength, view=view, M=M, entrance_pupil=entrance_pupil
                    )
                    ps, oss = self.trace_to_sensor(ray=ray, record=True)
    ...
    def plot_setup2D(
        ...
        if (
            len(self.surfaces) == 1
        ):  # if there is only one surface, then it should be aperture
            ...
            for i, s in enumerate(self.surfaces):
                ...
                if i < len(self.surfaces) - 1:
                    # Draw aperture
                    if (
                        self.materials[i].A < 1.0003
                        and self.materials[i + 1].A < 1.0003
                    ):  # both are AIR
                ...
                r = torch.linspace(
                    -s.r, s.r, s.APERTURE_SAMPLING, device=self.device
                )  # aperture sampling
    ...
    def draw_psf(
        ...
        for i in range(num_h):
            for j in range(num_w):
                psf = self.psf_diff_color(
                    o=points[i, j], kernel_size=ks, center=True, spp=spp
                )
                psf /= psf.max()
    ...
    def draw_psf_radial(
        ...
        for i in range(M):
            ...
            psf = self.psf_diff_color(
                o=points[i], kernel_size=ks, center=True, spp=4096
            )
            psf /= psf.max()
    ...
    def draw_spot_diagram(self, M=7, depth=DEPTH, save_name=None):
        ...
        # sample and trace rays
        ray = self.sample_point_source(
            M=M, R=self.sensor_size[0] / 2 / mag, depth=depth, spp=1024, pupil=True
        )
        ray, _, _ = self.trace(ray)
        ray.propagate_to(self.d_sensor)
    ...
    def loss_infocus(self, depth=DEPTH, M=31):
        ...
        for wv in [WAVE_RGB[0], WAVE_RGB[2]]:
            ...
            loss.append(
                torch.sum(p.abs() * ray.ra.unsqueeze(-1)) / torch.sum(ray.ra)
            )  # L1 loss

        L = (
            loss[0].detach() / loss[1].detach() * loss[0]
            + loss[1].detach() / loss[0].detach() * loss[1]
        )
        return L
    ...
    def analysis_rms(self, depth=DEPTH, ref=True):
        ...
        if ref:
            ...
            p_center_ref = (p_green * ray.ra.unsqueeze(-1)).sum(0) / ray.ra.sum(0).add(
                0.0001
            ).unsqueeze(-1)
        ...
        for wavelength in WAVE_RGB:
            ...
            if ref:
                ...
                o2_norm = (o2 - o2_center) * ray.ra.unsqueeze(
                    -1
    ...
    def get_optimizer(self, lrs=[1e-4, 1e-4, 0, 1e-4], iterations=100, decay=0.2):
        ...
        if c_ls and lrs[0] > 0:
            params.append(
                {"params": [self.surfaces[surf].c for surf in c_ls], "lr": lrs[0]}
            )
        if d_ls and lrs[1] > 0:
            params.append(
        ...
        if k_ls and lrs[2] > 0:
            params.append(
                {"params": [self.surfaces[surf].k for surf in k_ls], "lr": lrs[2]}
            )
        if lrs[3] > 0:
            if ai2_ls:
                params.append(
                    {
                        "params": [self.surfaces[surf].ai2 for surf in ai2_ls],
                        "lr": lrs[3] / decay,
                    }
                )
            if ai4_ls:
                params.append(
                    {
                        "params": [self.surfaces[surf].ai4 for surf in ai4_ls],
                        "lr": lrs[3],
                    }
                )
            if ai6_ls:
                params.append(
                    {
                        "params": [self.surfaces[surf].ai6 for surf in ai6_ls],
                        "lr": lrs[3] * decay,
                    }
                )
            if ai8_ls:
                params.append(
                    {
                        "params": [self.surfaces[surf].ai8 for surf in ai8_ls],
                        "lr": lrs[3] * decay**2,
                    }
                )
            if ai10_ls:
                params.append(
                    {
                        "params": [self.surfaces[surf].ai10 for surf in ai10_ls],
                        "lr": lrs[3] * decay**3,
                    }
                )
            if ai12_ls:
                params.append(
                    {
                        "params": [self.surfaces[surf].ai12 for surf in ai12_ls],
                        "lr": lrs[3] * decay**4,
                    }
                )
        if len(lrs) == 5 and lrs[4] > 0:
            params.append(
                {"params": [self.surfaces[surf].b3 for surf in b3_ls], "lr": lrs[4]}
            )
            params.append(
                {
                    "params": [self.surfaces[surf].b5 for surf in b5_ls],
                    "lr": lrs[4] * decay,
                }
            )
            params.append(
                {
                    "params": [self.surfaces[surf].b7 for surf in b7_ls],
                    "lr": lrs[4] * decay**2,
                }
            )
        ...
        scheduler = get_cosine_schedule_with_warmup(
            optimizer, num_warmup_steps=0.05 * iterations, num_training_steps=iterations
        )
    ...
    def refine(
        ...
        result_dir = (
            result_dir + "/" + datetime.now().strftime("%m%d-%H%M%S") + "-DesignLensRMS"
        )
        os.makedirs(result_dir, exist_ok=True)
        if not logging.getLogger().hasHandlers():
            set_logger(result_dir)
        logging.info(
            f"lr:{lrs}, decay:{decay}, iterations:{iterations}, spp:{spp}, M:{M}."
        )
    ...
    def write_lensfile(
        self, filename="./test.txt", str1="optimized lens file.\n", write_zmx=False
    ):
        ...
        f.writelines(
            f"I {self.d_sensor-self.surfaces[-1].d.item():.3f} {0.0} {self.r_last*2:.2f} {self.materials[-1].name}"
        )
        f.close()
...
def create_lens(
    ...
    partition = np.clip(
        np.random.randn(2 * surfnum - 1) + 1, 1, 2
    )  # consider aperture, position normalization
    partition = [
        p if i % 2 == 0 else (0.6 + 0.05 * i) * p for i, p in enumerate(partition)
    ]  # distance between lenses should be small
        ...
        if words[0] == "SURF":
            ...
            if surf_idx == 0 or surf_idx == 1:
                ...
                if surf_type == "STANDARD":
                    ...
                    surf = Aspheric(
                        r=surf_r, d=total_d - surf_d, c=surf_c, k=surf_k, ai=surf_ai
                    )
                    surfaces.append(surf)
                    mate = Material(name=surf_mate)
                    materials.append(mate)
