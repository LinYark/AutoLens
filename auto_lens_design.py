...
def curriculum_learning(lens, args):
    ...
    for step in range(args["curriculum_steps"] + 1):
        ...
        args["step"] = step
        diag1 = diag_start + (diag_target - diag_start) * np.sin(
            step / curriculum_steps * np.pi / 2
        )
        fnum1 = fnum_start + (fnum_target - fnum_start) * np.sin(
            step / curriculum_steps * np.pi / 2
        )
        lens = change_lens(lens, diag1, fnum1)

        lens.analysis(
            save_name=f"{result_dir}/step{step}_starting_point", zmx_format=True
        )
        lens.write_lensfile(
            f"{result_dir}/step{step}_starting_point.txt", write_zmx=True
        )
        logging.info(
            f"==> Curriculum learning step {step}, target: FOV {round(lens.hfov * 2 * 57.3, 2)}, DIAG {round(2 * lens.r_last, 2)}mm, F/{lens.fnum}."
        )
